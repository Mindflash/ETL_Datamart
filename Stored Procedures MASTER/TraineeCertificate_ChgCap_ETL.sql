
use pacifica ; 


/************ Stored Proc for ETL  **********************/

DROP PROCEDURE IF EXISTS TraineeCertificate_ChgCap_ETL;
DELIMITER $$
CREATE DEFINER=`sa`@`%` PROCEDURE `TraineeCertificate_ChgCap_ETL`(OUT SP_Result INT, OUT SP_MESSAGE varchar(150))
BEGIN


-- DECLARATIONS
	DECLARE new_extract_datetime datetime; 
	DECLARE previous_extract_datetime datetime;
		
	-- SQLEXCEPTION DECLARATION
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN	
			-- ERROR
            SET SP_Result = 1 ;
            SET SP_MESSAGE = 'An error has occurred, operation rollbacked and the stored procedure was terminated';
			ROLLBACK;
		END;
		

    
-- START TRANSACTION    
	START TRANSACTION;


-- SET VARIABLES
	SET new_extract_datetime = now(); 
	SET previous_extract_datetime =  (select last_extract_datetime from Control_Change_Capture where table_name = "TraineeCertificate")  ; 
	SET sp_result = 0;

-- MAIN PROCESS

	-- Truncate CC_TraineeCertificate
	Truncate Table CC_TraineeCertificate; 

	-- Insert to Change Capture (CC) Staging Table
		INSERT INTO CC_TraineeCertificate
		SELECT   
		 	new_extract_datetime as ETL_Run_Datetime,		-- record the datetime when this insert occurred.
		 	previous_extract_datetime,    	-- record the prior extract datetime and the new extract datetime.
			ID,
			AccountID,
			CourseID,
			Created,
			Modified,
			Deleted,
			CModuleID,
			UserID,
			QuizScore,
			CModuleCompleted

		FROM TraineeCertificate  
        WHERE Modified > Previous_Extract_Datetime    ;


	-- If Insert succeeded, then update the control table for this TraineeCertificate.
		Update Control_Change_Capture 
		Set Last_Extract_Datetime = New_Extract_Datetime
		where table_name = "TraineeCertificate" ; 


COMMIT;

END$$
DELIMITER ;







-- End of script