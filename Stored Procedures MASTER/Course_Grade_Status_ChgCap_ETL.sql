...
-- Change Capture E/L    For Course_Grade_Status Dim 
...
DROP PROCEDURE IF EXISTS Course_Grade_Status_ChgCap_ETL;


DELIMITER $$
CREATE DEFINER=`sa`@`%` PROCEDURE `Course_Grade_Status_ChgCap_ETL`(OUT SP_Result INT, OUT SP_MESSAGE varchar(150))
BEGIN
...
-- This is a drop/refresh process
	Truncate Table cc_Course_Grade_Status ; 

-- Insert to Change Capture (CC) Staging Table
	INSERT INTO CC_Course_Grade_Status
    (
	  ETL_Run_Datetime,
	  ID,  -- MTypeID for Course_Grade_Statuss
	  Name,   -- MTypeName
	  Description
	)
...
-- If Insert succeeded, then update the control table for this account.  In this case the record is only used for reference.
    ...
    where table_name = "Course_Grade_Status" ; 
