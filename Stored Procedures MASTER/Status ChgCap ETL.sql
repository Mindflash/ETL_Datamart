...
-- Change Capture E/L    For Status Dim 
...
DROP PROCEDURE IF EXISTS Status_ChgCap_ETL;


DELIMITER $$
CREATE DEFINER=`sa`@`%` PROCEDURE `Status_ChgCap_ETL`(OUT SP_Result INT, OUT SP_MESSAGE varchar(150))
BEGIN
...
-- Insert to Change Capture (CC) Staging Table
	INSERT INTO CC_Status
    (
	  ETL_Run_Datetime,
	  ID,  -- MTypeID for Statuss
	  Name,   -- MTypeName
	  Description
	)
